...
start_str = "              {(activeTab === 'income' ? incomeRecords : expenseRecords).map((record, index) => ("
end_str = "              )}            </motion.div>          </AnimatePresence>"
end_str = "              )}\n            </motion.div>\n          </AnimatePresence>"
# actually let's just find the start of the map block and the end of the length===0 block.
start_idx = content.find("              {(activeTab === 'income' ? incomeRecords : expenseRecords).map((record, index) => (")
if start_idx == -1:
    ...
    replacement = """              {(activeTab === 'income' ? incomeRecords : expenseRecords).length === 0 ? (
